...
// Iînëa ?aíäa?a ie?a e iîno-ýôôaeoîâ --#SM+#-- +SecondVP+
void CRender::AfterWorldRender()
{
	if (Device.m_SecondViewport.IsSVPFrame())
	{
		// Äaëaao eîie? áýeáóôa?a (oaeóuaaî ýe?aía) â ?aíäa?-oa?aao âoî?îaî âü?iî?oa
		IRender_Target* T = getTarget();
		IDirect3DSurface9* pBackBuffer = nullptr;
		HW.pDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer); // Iîëó÷aai nnuëeó ía áýeáóôa?
		D3DXLoadSurfaceFromSurface(Target->RT_SecondVP->pRT, 0, 0, pBackBuffer, 0, 0, D3DX_DEFAULT, 0);
		pBackBuffer->Release(); // Eî??aeoíî î÷euaai nnuëeó ía áýeáóôa? (eía÷a ea?a çaâeníao â îiöeyo)
	}
}